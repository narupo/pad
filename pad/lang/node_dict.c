...
PadNodeDict_EscDel(PadNodeDict *self) {
    ...
    PadNodeDictItem *map = PadMem_Move(self->map);
    self->map = NULL;
    free(self);
...
PadNodeDict_New(void) {
    PadNodeDict *self = PadMem_Calloc(1, sizeof(*self));
    if (!self) {
        return NULL;
    }
    ...
    self->len = 0;
    self->map = PadMem_Calloc(self->capa+1, sizeof(PadNodeDictItem));
    if (!self->map) {
...
PadNodeDict_Resize(PadNodeDict *self, int32_t newcapa) {
    ...
    PadNodeDictItem *tmpmap = PadMem_Realloc(self->map, newcapa*byte + byte);
    if (!tmpmap) {
        return NULL;
    }
...
PadNodeDict_Move(PadNodeDict *self, const char *key, struct PadNode *move_value) {
    ...
    PadCStr_Copy(el->key, PAD_NODE_DICT__ITEM_KEY_SIZE, key);
    el->value = move_value;
...
PadNodeDict_Pop(PadNodeDict *self, const char *key) {
    ...
    for (int32_t i = found_index; i < self->len - 1; ++i) {
        ...
        PadCStr_Copy(cur->key, PAD_NODE_DICT__ITEM_KEY_SIZE, next->key);
