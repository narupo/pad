...
#define pushb_error(ast, node, fmt, ...) \
    errstack_pushb(ast->error_stack, NULL, 0, NULL, 0, fmt, ##__VA_ARGS__)
...
trv_ref_block(ast_t *ast, trv_args_t *targs) {
    ...
    switch (result->type) {
    default:
        pushb_error(ast, NULL, "can't refer object (%d)", result->type);
        break;
    case OBJ_TYPE_NIL:
...
trv_import_as_stmt(ast_t *ast, trv_args_t *targs) {
    ...
    if (!pathobj || pathobj->type != OBJ_TYPE_UNICODE) {
        pushb_error(ast, NULL, "invalid path object in import as statement");
        return_trav(NULL);
    }
    ...
    if (!aliasobj || aliasobj->type != OBJ_TYPE_IDENTIFIER) {
        pushb_error(ast, NULL, "invalid identifier object in import as statement");
        obj_del(pathobj);
        obj_del(aliasobj);
        return_trav(NULL);
    }
    ...
    if (!importer_import_as(
        ...
        pushb_error(ast, NULL, importer_getc_error(importer));
        obj_del(pathobj);
        obj_del(aliasobj);
        return_trav(NULL);
    }
...
trv_from_import_stmt(ast_t *ast, trv_args_t *targs) {
    ...
    if (!pathobj || pathobj->type != OBJ_TYPE_UNICODE) {
        pushb_error(ast, NULL, "invalid path object in from import statement");
        obj_del(pathobj);
        return_trav(NULL);
    }
    ...
    if (!varsobj || varsobj->type != OBJ_TYPE_ARRAY) {
        pushb_error(ast, NULL, "invalid variables object in from import statement");
        obj_del(pathobj);
        obj_del(varsobj);
        return_trav(NULL);
    }
...
trv_import_vars(ast_t *ast, trv_args_t *targs) {
    ...
    for (int32_t i = 0; i < nodearr_len(nodearr); ++i) {
        ...
        if (!varobj || varobj->type != OBJ_TYPE_ARRAY) {
            pushb_error(ast, NULL, "invalid variable object in import variables");
            objarr_del(objarr);
            return_trav(NULL);
        }
...
trv_import_var(ast_t *ast, trv_args_t *targs) {
    ...
    if (!idnobj || idnobj->type != OBJ_TYPE_IDENTIFIER) {
        pushb_error(ast, NULL, "invalid identifier object in import variable");
        obj_del(idnobj);
        return_trav(NULL);
    }
    ...
    if (aliasobj) {
        if (aliasobj->type != OBJ_TYPE_IDENTIFIER) {
            pushb_error(ast, NULL, "invalid alias object in import variable");
            obj_del(idnobj);
            obj_del(aliasobj);
            return_trav(NULL);
        }
...
trv_if_stmt(ast_t *ast, trv_args_t *targs) {
    ...
    if (!result) {
        pushb_error(ast, NULL, "traverse error. test return null in if statement");
        return_trav(NULL);
    }

    bool boolean = parse_bool(ast, result);
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to parse boolean");
        return_trav(NULL);
    }
    result = NULL;
    ...
    if (boolean) {
        for (int32_t i = 0; i < nodearr_len(if_stmt->contents); ++i) {
            ...
            result = _trv_traverse(ast, targs);
            if (ast_has_errors(ast)) {
                pushb_error(ast, NULL, "failed to execute contents in if-statement");
                return_trav(NULL);
            }
        }
    } else {
...
trv_else_stmt(ast_t *ast, trv_args_t *targs) {
    ...
    for (int32_t i = 0; i < nodearr_len(else_stmt->contents); ++i) {
        ...
        result = _trv_traverse(ast, targs);
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to execute contents in else-statement");
            return_trav(NULL);
        }
    }
...
trv_return_stmt(ast_t *ast, trv_args_t *targs) {
    ...
    object_t *result = _trv_traverse(ast, targs);
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to traverse formula");
        return_trav(NULL);
    }
    if (!result) {
        pushb_error(ast, NULL, "result is null from formula in return statement");
        return_trav(NULL);
    }
...
again:
    switch (result->type) {
    default:
        pushb_error(ast, NULL, "invalid return type (%d)", result->type);
        return NULL;
        break;
    case OBJ_TYPE_CHAIN:
...
trv_block_stmt(ast_t *ast, trv_args_t *targs) {
    ...
    if (!idn || ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to traverse identifier");
        return_trav(NULL);
    }
    ...
    for (int32_t i = 0; i < nodearr_len(contents); ++i) {
        ...
        object_t *result = _trv_traverse(ast, targs);
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to traverse content");
            goto fail;
        }
        obj_del(result);
    }
...
trv_inject_stmt(ast_t *ast, trv_args_t *targs) {
    ...
    if (!targs->func_obj) {
        pushb_error(ast, NULL, "can't inject in out of function");
        return_trav(NULL);
    }
    ...
    if (!extends_func) {
        pushb_error(ast, NULL, "can't inject. not found extended function");
        return_trav(NULL);
    }
    ...
    for (;;) {
        ...
        if (!item) {
            if (!func->extends_func) {
                pushb_error(ast, NULL, "not found \"%s\" block", idnname);
                return_trav(NULL);
            }
...
trv_def_struct(ast_t *ast, trv_args_t *targs) {
    ...
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to traverse elems in struct");
        return_trav(NULL);
    }
    assert(!result);
    ...
    if (!def_struct) {
        pushb_error(ast, NULL, "failed to create def-struct object");
        return_trav(NULL);
    }
    ...
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to move object");
        return_trav(NULL);
    }
...
trv_content(ast_t *ast, trv_args_t *targs) {
    ...
    if (content->elems) {
        ...
        result = _trv_traverse(ast, targs);
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to traverse elems");
            return_trav(NULL);
        }
        ...
        result = _trv_traverse(ast, targs);
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to traverse blocks");
            return_trav(NULL);
        }
    } else {
        pushb_error(ast, NULL, "invalid status of content");
    }
...
again:
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "invalid right operand (%d)", rhs->type);
        return_trav(NULL);
        break;
    case OBJ_TYPE_CHAIN: {
        rhs = refer_chain_obj_with_ref(ast, rhs);
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to refer chain object");
            return_trav(NULL);
        }
        goto again;
    } break;
    case OBJ_TYPE_ARRAY: {
        ...
        if (objarr_len(lhs->objarr) != objarr_len(rhs->objarr)) {
            pushb_error(ast, NULL, "can't assign array to array. not same length");
            return_trav(NULL);
        }
...
trv_assign_to_chain(ast_t *ast, trv_args_t *targs) {
    ...
    for (int32_t i = 0; i < coslen-1; ++i) {
        ...
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to refer three objects");
            return NULL;
        }
        assert(last);
        obj_inc_ref(last);
    ...
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to assign to three objects");
        return NULL;
    }
...
trv_calc_assign(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default:
        pushb_error(ast, NULL, "invalid left hand operand (%d)", lhs->type);
        return_trav(NULL);
        break;
    case OBJ_TYPE_IDENTIFIER: {
...
trv_simple_assign(ast_t *ast, trv_args_t *targs) {
    ...
    if (!nodearr_len(simple_assign->nodearr)) {
        pushb_error(ast, NULL, "failed to traverse simple assign. array is empty");
        return_trav(NULL);
    }
...
trv_assign(ast_t *ast, trv_args_t *targs) {
    ...
    if (!nodearr_len(assign_list->nodearr)) {
        pushb_error(ast, NULL, "failed to traverse assign. array is empty");
        return_trav(NULL);
    }
...
#define _return(result) \
    ...
    for (int32_t i = arrlen-2; i >= 0; --i) {
        ...
        // why lhs in null?
        if (!lhs) {
            pushb_error(ast, NULL, "left hand side object is null");
            _return(NULL);
        }
...
trv_assign_list(ast_t *ast, trv_args_t *targs) {
    ...
    if (!nodearr_len(assign_list->nodearr)) {
        pushb_error(ast, NULL, "failed to traverse assign list. array is empty");
        return_trav(NULL);
    }
...
trv_multi_assign(ast_t *ast, trv_args_t *targs) {
    ...
    for (int32_t i = arrlen-2; i >= 0; --i) {
        ...
        if (!lhs) {
            pushb_error(ast, NULL, "failed to traverse left test_list in multi assign");
            return_trav(NULL);
        }
        ...
        if (!result) {
            pushb_error(ast, NULL, "failed to assign in multi assign");
            return_trav(NULL);
        }
...
trv_call_args(ast_t *ast, trv_args_t *targs) {
    ...
    for (int32_t i = 0; i < nodearr_len(call_args->nodearr); ++i) {
        ...
        object_t *result = _trv_traverse(ast, targs);
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to traverse call argument");
            return_trav(NULL);
        }
        assert(result);
        ...
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to extract reference");
            return_trav(NULL);
        }
...
trv_compare_or_int(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        ...
        if (!rval) {
            pushb_error(ast, NULL, "can't compare or int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_or_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare or bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_or_string(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare or string. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_or_array(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare or array. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_or_dict(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare or dict. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_or_nil(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare or nil. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_or_func(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare or func. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_or(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        ...
        if (!lval) {
            pushb_error(ast, NULL, "can't compare or. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and_int(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare and int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare and bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and_string(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare and string. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and_array(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare and array. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and_dict(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare and dict. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and_nil(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare and nil. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and_func(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't compare and func. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_and(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't compare and. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_not(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        ...
        if (!val) {
            pushb_error(ast, NULL, "can't compare not. index object value is null");
            return_trav(NULL);
        }
...
trv_not_test(ast_t *ast, trv_args_t *targs) {
    ...
    if (not_test->not_test) {
        ...
        if (!operand) {
            pushb_error(ast, NULL, "failed to not test");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_int(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_string(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq unicode. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_array(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq array. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_dict(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq dict. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_nil(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq nil. index object value is null");
            return_trav(NULL);
        }
        ...
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to compare not equal to nil");
            return_trav(NULL);
        }
        return_trav(obj);
    } break;
...
trv_compare_comparison_eq_func(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq func. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_def_struct(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison eq struct. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_eq_chain(ast_t *ast, trv_args_t *targs) {
    ...
    object_t *lval = extract_ref_of_obj(ast, lhs);
    if (!lval) {
        pushb_error(ast, NULL, "chain object value is null");
        return_trav(NULL);
    }
...
trv_compare_comparison_not_eq_int(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq_unicode(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq string. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq_array(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq array. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq_dict(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq dict. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq_nil(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq nil. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq_func(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq func. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq_def_struct(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison not eq struct. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_not_eq(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't comparison not eq. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_lte_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare lte with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_bool(ast->ref_gc, lhs->lvalue <= rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison lte int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_lte_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison lte bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_lte(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare with lte");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't comparison lte. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_gte_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare gte with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_bool(ast->ref_gc, lhs->lvalue >= rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison gte int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_gte_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison gte bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_gte(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare with gte");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't comparison gte. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_lt_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare lt with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_bool(ast->ref_gc, lhs->lvalue < rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison lt int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_lt_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison lt bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_lt(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare with lt (%d)", lhs->type);
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't comparison lt. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_gt_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare gt with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_bool(ast->ref_gc, lhs->lvalue > rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison gt int. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_gt_bool(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't comparison gt bool. index object value is null");
            return_trav(NULL);
        }
...
trv_compare_comparison_gt(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default:
        pushb_error(ast, NULL, "can't compare with gt");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't comparison gt. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_expr_add_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't add with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't add with int. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_expr_add_bool(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't add with bool");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't add with bool. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_expr_add_string(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't add %d with string", rhs->type);
        return_trav(NULL);
        break;
    case OBJ_TYPE_IDENTIFIER: {
        check("call trv_roll_identifier_rhs");
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't add with string. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_expr_add_array(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rref->type) {
    default: {
        pushb_error(ast, NULL, "invalid right hand operand (%d)", rref->type);
        return NULL;
    } break;
    case OBJ_TYPE_ARRAY: {
...
trv_calc_expr_add(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default: {
        pushb_error(ast, NULL, "can't add");
        return_trav(NULL);
    } break;
    case OBJ_TYPE_INT: {
...
trv_calc_expr_sub_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't sub with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_int(ast->ref_gc, lhs->lvalue - rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't sub with int. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_expr_sub_bool(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't sub with bool");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_int(ast->ref_gc, lhs->boolean - rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't sub with bool. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_expr_sub(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default: {
        pushb_error(ast, NULL, "can't sub");
        return_trav(NULL);
    } break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't sub. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_term_mul_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't mul with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_int(ast->ref_gc, lhs->lvalue * rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't mul with int. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_term_mul_bool(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't mul with bool");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        object_t *obj = obj_new_int(ast->ref_gc, lhs->boolean * rhs->lvalue);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't mul with bool. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_term_mul_string(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't mul with string");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't mul with string. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_term_mul(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default: {
        pushb_error(ast, NULL, "can't mul");
        return_trav(NULL);
    } break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *lval = extract_ref_of_obj(ast, lhs);
        if (!lval) {
            pushb_error(ast, NULL, "can't mul. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_term_div_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't division with int");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT: {
        if (!rhs->lvalue) {
            pushb_error(ast, NULL, "zero division error");
            return_trav(NULL);
        }
        object_t *obj = obj_new_int(ast->ref_gc, lhs->lvalue / rhs->lvalue);
        return_trav(obj);
    ...
    case OBJ_TYPE_BOOL: {
        if (!rhs->boolean) {
            pushb_error(ast, NULL, "zero division error (2)");
            return_trav(NULL);
        }
        object_t *obj = obj_new_int(ast->ref_gc, lhs->lvalue / rhs->boolean);
        return_trav(obj);
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't division with int. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_term_div_bool(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhs->type) {
    default:
        pushb_error(ast, NULL, "can't division with bool");
        return_trav(NULL);
        break;
    case OBJ_TYPE_INT:
        if (!rhs->lvalue) {
            pushb_error(ast, NULL, "zero division error (3)");
            return_trav(NULL);
        }
        return obj_new_int(ast->ref_gc, lhs->boolean / rhs->lvalue);
    case OBJ_TYPE_BOOL:
        if (!rhs->boolean) {
            pushb_error(ast, NULL, "zero division error (4)");
            return_trav(NULL);
        }
    ...
    case OBJ_TYPE_CHAIN: {
        object_t *rval = extract_ref_of_obj(ast, rhs);
        if (!rval) {
            pushb_error(ast, NULL, "can't division with bool. index object value is null");
            return_trav(NULL);
        }
...
trv_calc_term_div(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhs->type) {
    default: {
        pushb_error(ast, NULL, "can't division");
        return_trav(NULL);
    } break;
    case OBJ_TYPE_INT: {
    ...
    case OBJ_TYPE_CHAIN: {
        ...
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "can't division. index object value is null");
            return_trav(NULL);
        }
        assert(lval);
...
trv_calc_term_mod_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhsref->type) {
    default:
        pushb_error(ast, NULL, "invalid right hand operand (%d)", rhsref->type);
        return_trav(NULL);
        break;
    case OBJ_TYPE_BOOL: {
        if (!rhsref->boolean) {
...
trv_calc_term_mod(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhsref->type) {
    default:
        pushb_error(ast, NULL, "invalid left hand operand (%d)", lhsref->type);
        return_trav(NULL);
        break;
    case OBJ_TYPE_BOOL: {
...
trv_negative(ast_t *ast, trv_args_t *targs) {
    ...
    if (!operand) {
        pushb_error(ast, NULL, "not found operand in negative");
        return_trav(NULL);
    }
    ...
    default:
        if (negative->is_negative) {
            pushb_error(ast, NULL, "invalid operand type (%d) in negative", operand->type);
            return_trav(NULL);
        }
        return operand;
    break;
...
trv_chain(ast_t *ast, trv_args_t *targs) {
    ...
    object_t *operand = _trv_traverse(ast, targs);
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to traverse factor");
        return_trav(NULL);
    }
    assert(operand);
    ...
    for (int32_t i = 0; i < chain_nodes_len(cns); ++i) {
        ...
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to traverse node");
            goto fail;
        }
        ...
        default:
            pushb_error(ast, NULL, "invalid chain node type (%d)", chain_node_getc_type(cn));
            goto fail;
            break;
        }
...
trv_calc_asscalc_add_ass_identifier_int(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_IDENTIFIER: {
        ...
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to extract object");
            return_trav(NULL);
        }
...
trv_calc_asscalc_add_ass_chain(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lref->type) {
    default: {
        pushb_error(ast, NULL, "invalid left hand operand (%d)", lref->type);
        return NULL;
    } break;
    case OBJ_TYPE_INT: {
        switch (rref->type) {
...
trv_calc_asscalc_sub_ass_idn_int(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhsref->type) {
    default: {
        pushb_error(ast, NULL, "invalid right hand operand type (%d)", rhsref->type);
        return_trav(NULL);
    } break;
    case OBJ_TYPE_INT: {
...
trv_calc_asscalc_sub_ass_idn(ast_t *ast, trv_args_t *targs) {
    ...
    object_t *lhsref = extract_ref_of_obj(ast, lhs);
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to pull reference");
        return_trav(NULL);
    }

    switch (lhsref->type) {
    default: {
        pushb_error(ast, NULL, "invalid left hand operand type (%d)", lhs->type);
        return_trav(NULL);
    } break;
    case OBJ_TYPE_INT: {
        targs->lhs_obj = lhsref;
...
trv_calc_asscalc_mul_ass_string(ast_t *ast, trv_args_t *targs) {
    ...
    case OBJ_TYPE_INT: {
        if (rhsref->lvalue < 0) {
            pushb_error(ast, NULL, "can't mul by negative value");
            return_trav(NULL);
...
trv_calc_asscalc_div_ass_bool(ast_t *ast, trv_args_t *targs) {
    ...
    switch (rhsref->type) {
    default: {
        pushb_error(ast, NULL, "invalid right hand operand");
        return_trav(NULL);
    } break;
    case OBJ_TYPE_INT: {
        if (rhsref->lvalue == 0) {
...
trv_calc_asscalc_div_ass(ast_t *ast, trv_args_t *targs) {
    ...
    switch (lhsref->type) {
    default: {
        pushb_error(ast, NULL, "invalid left hand operand");
        return_trav(NULL);
    } break;
    case OBJ_TYPE_BOOL: {
...
trv_array_elems(ast_t *ast, trv_args_t *targs) {
    ...
    for (int32_t i = 0; i < nodearr_len(array_elems->nodearr); ++i) {
        ...
        object_t *result = _trv_traverse(ast, targs);
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "result is null");
            return_trav(NULL);
        }
...
trv_dict_elem(ast_t *ast, trv_args_t *targs) {
    ...
    switch (key->type) {
    default:
        pushb_error(ast, NULL, "key is not string in dict elem");
        return_trav(NULL);
        break;
    case OBJ_TYPE_UNICODE:
    case OBJ_TYPE_IDENTIFIER:
...
trv_dict_elems(ast_t *ast, trv_args_t *targs) {
    ...
    for (int32_t i = 0; i < nodearr_len(dict_elems->nodearr); ++i) {
        ...
        switch (key->type) {
        default:
            pushb_error(ast, NULL, "invalid key type");
            obj_del(arrobj);
            objdict_del(objdict);
            return_trav(NULL);
            break;
        ...
        case OBJ_TYPE_IDENTIFIER: {
            ...
            if (ref->type != OBJ_TYPE_UNICODE) {
                pushb_error(ast, NULL, "invalid key type in variable of dict");
                obj_del(arrobj);
                objdict_del(objdict);
                return_trav(NULL);
                break;
...
trv_identifier(ast_t *ast, trv_args_t *targs) {
    ...
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to get ast by owner");
        return_trav(NULL);
    }
...
trv_func_def(ast_t *ast, trv_args_t *targs) {
    ...
    if (!name) {
        ...
        pushb_error(ast, NULL, "failed to traverse name in traverse func def");
        return_trav(NULL);
    }
    ...
    if (ast_has_errors(ast)) {
        pushb_error(ast, NULL, "failed to traverse func def params");
        return_trav(NULL);
    }
    assert(def_args);
    ...
    if (func_def->func_extends) {
        ...
        if (ast_has_errors(ast)) {
            pushb_error(ast, NULL, "failed to traverse func-extends");
            return_trav(NULL);
        }
...
trv_traverse(ast_t *ast, context_t *context) {
    ...
    if (!trv_import_builtin_modules(ast)) {
        pushb_error(ast, NULL, "failed to import builtin modules");
        return;
    }
