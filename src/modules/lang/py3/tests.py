...
@}{{ v }}'''))
    ...
    def test_ast_call_stmt(self):
        ...
        with self.assertRaises(AST.SyntaxError): # TODO
            a.parse(t.parse('''{@
                def f():
                    return 1
                end
...
{@ v = "v" @} '''))
        ...
        with self.assertRaises(AST.SyntaxError):
            ...
            self.assertEqual(c.last_expr_val, (0, 1, 2, 3))
    ...
    def test_ast_assign_expr(self):
        ...
        with self.assertRaises(AST.SyntaxError):
            ...
            self.assertEqual(c.last_expr_val, 1)
            self.assertEqual(a.current_scope.syms['a'], 1)
            self.assertEqual(a.current_scope.syms['b'], None)

        a.parse(t.parse('''{@
            a, b = 1+2, 2+3
        @}'''))
        c = a.traverse()
        self.assertEqual(c.last_expr_val, (3, 5))
        self.assertEqual(a.current_scope.syms['a'], 3)
        self.assertEqual(a.current_scope.syms['b'], 5)
        ...
        c = a.traverse()
        self.assertEqual(c.last_expr_val, (1, 2, 3))
        self.assertEqual(a.current_scope.syms['a'], (1, 2, 3))
...
@}{{ a }}'''))
    ...
    def test_ast_nil(self):
        ...
        a.parse(t.parse('{@ nil @}'))
        c = a.traverse()
        self.assertEqual(c.last_expr_val, None)

        a.parse(t.parse('{{ nil }}'))
        c = a.traverse()
        self.assertEqual(c.last_expr_val, None)

        a.parse(t.parse('{{ a = nil }}'))
        c = a.traverse()
        self.assertEqual(c.last_expr_val, None)
        self.assertEqual(a.current_scope.syms['a'], None)

        a.parse(t.parse('{{ a = a = nil }}'))
        c = a.traverse()
        self.assertEqual(c.last_expr_val, None)
        self.assertEqual(a.current_scope.syms['a'], None)

        with self.assertRaises(AST.SyntaxError):
            a.parse(t.parse('{{ a = nil = nil }}'))
            c = a.traverse()
            self.assertEqual(c.last_expr_val, None)
            self.assertEqual(a.current_scope.syms['a'], None)
