...
test_ast_parse(void) {
    ...
    program = root->real;
    assert(program->block != NULL);
    assert(program->block->type == NODE_TYPE_BLOCK);
    assert(program->block->real != NULL);
    assert(program->program == NULL);
    block = program->block->real;
    assert(block->code_block != NULL);
    assert(block->code_block->type == NODE_TYPE_CODE_BLOCK);
    assert(block->code_block->real != NULL);
    assert(block->ref_block == NULL);
    assert(block->text_block == NULL);
    code_block = block->code_block->real;
    ...
    ***************/
    ...
    assert(program->block->real != NULL);
    assert(program->program != NULL);
    assert(program->program->type == NODE_TYPE_PROGRAM);
    assert(program->program->real != NULL);
    block = program->block->real;
    assert(block->code_block == NULL);
    assert(block->ref_block == NULL);
    assert(block->text_block != NULL);
    assert(block->text_block->type == NODE_TYPE_TEXT_BLOCK);
    assert(block->text_block->real != NULL);
    text_block = block->text_block->real;
    assert(text_block->text != NULL);
    assert(!strcmp(text_block->text, "abc"));
    program = program->program->real;
    assert(program->block != NULL);
    assert(program->block->type == NODE_TYPE_BLOCK);
    assert(program->block->real != NULL);
    assert(program->program != NULL);
    ...
    assert(if_stmt->elems == NULL);
    assert(if_stmt->block != NULL);
    assert(if_stmt->block->type == NODE_TYPE_BLOCK);
    assert(if_stmt->block->real != NULL);
    assert(if_stmt->elif_stmt == NULL);
    assert(if_stmt->else_stmt == NULL);
    block = if_stmt->block->real;
    assert(block->code_block == NULL);
    assert(block->ref_block == NULL);
    assert(block->text_block != NULL);
    assert(block->text_block->type == NODE_TYPE_TEXT_BLOCK);
