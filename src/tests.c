...
test_ast_parse(void) {
    ...
    *******/
    ...
    tkr_parse(tkr, "{@ 1 * 2 @}");
    {
        ...
        assert(digit != NULL);
        assert(digit->lvalue == 1);
        asscalc = nodearr_get(term->nodearr, 2)->real;
        factor = nodearr_get(asscalc->nodearr, 0)->real;
        atom = factor->atom->real;
        digit = atom->digit->real;
        assert(digit != NULL);
    ...
    tkr_parse(tkr, "{@ 1 * 2 * 3 @}");
    {
        ...
        assert(digit != NULL);
        assert(digit->lvalue == 2);    
        asscalc = nodearr_get(term->nodearr, 4)->real;
        factor = nodearr_get(asscalc->nodearr, 0)->real;
        atom = factor->atom->real;
        digit = atom->digit->real;
        assert(digit != NULL);
    ...
    *******/
    ...
    tkr_parse(tkr, "{@ 1 @}");
    {
        ...
        assign_list = formula->assign_list->real;
        assert(nodearr_get(assign_list->nodearr, 0) != NULL);
        assert(nodearr_get(assign_list->nodearr, 0)->type == NODE_TYPE_TEST_LIST);
...
test_ast_traverse(void) {
    ...
    **************/
    ...
    tkr_parse(tkr, "{@ a = alias.set(\"\", \"\")\n b = alias.set(\"\", \"\") @}{: a :}{: b :}");
    {
        ...
        ast_traverse(ast, ctx);
        assert(!ast_has_error(ast));
        assert(!strcmp(ctx_getc_buf(ctx), "nullnull"));
    } 
