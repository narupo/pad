...
rmcmd_parse_opts(rmcmd_t *self) {
    ...
    for (;;) {
        ...
        case '?':
        default:
            cstr_appfmt(self->what, sizeof self->what, "unknown option.");
    ...
    if (self->argc < optind) {
        cstr_appfmt(self->what, sizeof self->what, "failed to parse option.");
...
rmcmd_remove_r(rmcmd_t *self, const char *dirpath) {
    if (is_out_of_home(self->config->home_path, dirpath)) {
        cstr_appfmt(self->what, sizeof self->what, "\"%s\" is out of home.", dirpath);
        self->errno_ = RMCMD_ERR_OUTOFHOME;
        return false;
    }

    if (!file_isdir(dirpath)) {
        cstr_appfmt(self->what, sizeof self->what, "\"%s\" is not a directory.", dirpath);
    ...
    if (!dir) {
        cstr_appfmt(self->what, sizeof self->what, "failed to open directory \"%s\".", dirpath);
        self->errno_ = RMCMD_ERR_OPENDIR;
        return false;
    }
    ...
    for (file_dirnode_t *node; (node = file_dirread(dir)); ) {
        ...
        if (!file_solvefmt(path, sizeof path, "%s/%s", dirpath, dirname)) {
            cstr_appfmt(self->what, sizeof self->what, "failed to solve path by \"%s\".", dirname);
        ...
        if (file_isdir(path)) {
            ...
            if (file_remove(path) != 0) {
                cstr_appfmt(self->what, sizeof self->what, "failed to remove directory \"%s\".", path);
                self->errno_ = RMCMD_ERR_REMOVE_FILE;
                return false;
            }            
        } else {
            if (file_remove(path) != 0) {
                cstr_appfmt(self->what, sizeof self->what, "failed to remove file \"%s\".", path);
                self->errno_ = RMCMD_ERR_REMOVE_FILE;
                return false;
            }
        }
    ...
    if (file_dirclose(dir) != 0) {
        cstr_appfmt(self->what, sizeof self->what, "failed to close directory \"%s\".", dirpath);
...
rmcmd_rmr(rmcmd_t *self) {
    for (int i = self->optind; i < self->argc; ++i) {
        ...
        if (!rmcmd_remove_r(self, path)) {
            cstr_appfmt(self->what, sizeof self->what, "could not delete recusively.");
            return 1;
        }
